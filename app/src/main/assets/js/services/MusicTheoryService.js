...
const MusicTheoryService = {
    ...
    getDiatonicChordSuggestions({ tonic = 'C', scaleName = 'major', currentChordTokenized = null }) {
        if (!this.isTonalJsLoaded || !Tonal.Scale.chords) {
            console.warn("[MTS.getDiatonicChordSuggestions] Tonal.Scale.chords function not available.");
            return [];
        }
        try {
            const scaleChordsSymbols = Tonal.Scale.chords(scaleName);
            const tonicPc = Tonal.Note.pitchClass(tonic);

            if (!scaleChordsSymbols || scaleChordsSymbols.length === 0) {
                return [];
            }
            
            const suggestions = scaleChordsSymbols.map(chordSymbol => {
                const chordInKey = Tonal.Chord.transpose(chordSymbol, tonicPc);
                const chordInfo = Tonal.Chord.get(chordInKey);

                if (chordInfo.empty) return null;

                const octave = Tonal.Note.get(tonic).oct || 4; // Используем октаву тоники или 4 по умолчанию
                const id = `${chordInfo.tonic}${octave}${chordInfo.type}`;
                
                return {
                    id: id,
                    nameForService: id, // nameForService теперь совпадает с id, как в изначальном плане
                    displayName: `${chordInfo.tonic} ${chordInfo.aliases[0] || chordInfo.type}`
                };
            }).filter(Boolean);
            ...
            console.error(`[MTS] Error in getDiatonicChordSuggestions:`, e);
            return [];
        }
    }
};
